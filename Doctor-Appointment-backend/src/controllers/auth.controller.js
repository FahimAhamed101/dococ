...
const generateUserName = async (firstName, lastName) => {
  let baseUserName = `${firstName.replace(/\s+/g, '').toLowerCase()}${lastName.replace(/\s+/g, '').toLowerCase()}`;

  let userName = baseUserName;
  let userCount = 1;

  while (await userService.getUserByUserName(userName)) {
    userName = `${baseUserName}${userCount}`;
    userCount += 1;
  }
...
const register = catchAsync(async (req, res) => {
  ...
  if (isUser) {
    if (isUser.isDeleted) {
      user = await userService.isUpdateUser(isUser.id, {
...
const login = catchAsync(async (req, res) => {
  ...
  res.cookie('accessToken', tokens.access.token, cookieOptions).status(httpStatus.OK).json(
    response({
